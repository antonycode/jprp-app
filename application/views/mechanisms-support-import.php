...
<script>
    function mechanismsUpload() {
        ...
        $.ajax({
            type:'POST',
            url:'<?php echo base_url("supportimport/supportexcelimport"); ?>',
...
{% for (var i=0, file; file=o.files[i]; i++) { %}
    <tr class="template-upload fade">
        ...
        <input type="hidden" id="filename_id"value="{%=file.name%}"/>
        <td>
        ...
        <td>
            {% if (!i && !o.options.autoUpload) { %}
                <button class="btn btn-primary start" onclick="mechanismsUpload()" disabled>
                    <i class="glyphicon glyphicon-upload"></i>
                    <span>Start</span>
                </button>
            {% } %}
